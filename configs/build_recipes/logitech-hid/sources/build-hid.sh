...
if [[ -f "$KCONFIG" ]]; then
  for mod in HID_LOGITECH_DJ HID_LOGITECH_HIDPP; do
    ...
    case "$val" in
      ...
      y) echo "ERROR: CONFIG_${mod}=y (built-in) — cannot replace" >&2; exit 1 ;;
...
for mod in hid-logitech-dj hid-logitech-hidpp; do
  ko="$BUILD_DIR/$mod.ko"
  [[ -s "$ko" ]] || { echo "ERROR: $mod.ko missing or empty" >&2; exit 1; }
  modinfo "$ko" >/dev/null 2>&1 || { echo "ERROR: $mod.ko is not a valid module" >&2; exit 1; }
  vermagic="$(modinfo -F vermagic "$ko" 2>/dev/null | head -1)"
  [[ "$vermagic" == "$KVER "* ]] || { echo "ERROR: $mod.ko vermagic '$vermagic' != $KVER" >&2; exit 1; }
...
modinfo -F alias "$BUILD_DIR/hid-logitech-dj.ko" | grep -qi 'v0000046Dp0000C547' \
  || { echo "ERROR: hid-logitech-dj lacks 046d:c547 alias" >&2; exit 1; }
...
for mod in hid-logitech-dj hid-logitech-hidpp; do
  ...
  [[ "$installed_path" == */updates/logitech/* ]] \
    || { echo "ERROR: $mod resolves to $installed_path — not /updates/logitech/" >&2; exit 1; }
