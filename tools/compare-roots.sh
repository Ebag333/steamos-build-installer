...
for entry in "${rootfs_parts[@]}"; do
    ...
    case "$label" in
        rootfs-A) PART_A="$dev"; LABEL_A="$label" ;;
        rootfs-B) PART_B="$dev"; LABEL_B="$label" ;;
    esac
done
...
mount_ro "$PART_A" "$MNT_A" || { echo "Failed to mount A"; exit 1; }
mount_ro "$PART_B" "$MNT_B" || { echo "Failed to mount B"; exit 1; }
