...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --case)          EXTRA_CASES+=("$2"); shift 2 ;;
    --no-mangohud)   USE_MANGOHUD=0; shift ;;
    -h|--help)       usage ;;
    -*)              echo "Unknown option: $1" >&2; exit 1 ;;
    *)
      [[ -z "$APPID" ]] || { echo "Unexpected argument: $1" >&2; exit 1; }
      APPID="$1"; shift ;;
  esac
done

[[ -n "$APPID" ]] || { echo "Error: appid required" >&2; usage; }
...
[[ -x "$PROTONDB_SCRIPT" ]] || {
  echo "ERROR: protondb_launch_options_v8.py not found at $PROTONDB_SCRIPT" >&2; exit 1
}
...
for i, c in enumerate(candidates[:10], 1):
    ...
    print(f'{i:2}. [{score:5.1f}] ({reports:3} reports) {canon}')
    if not applicable:
