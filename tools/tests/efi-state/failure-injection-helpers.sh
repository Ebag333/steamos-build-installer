...
# ===========================================================================
# _fi_send_signal_at_phase SIGNAL PHASE_FILE TIMEOUT_SECS
#
...
#
#   # In the caller:
#   _fi_send_signal_at_phase SIGTERM "$PHASE_FILE" 5
#
# Arguments:
...
# ===========================================================================
_fi_send_signal_at_phase() {
  local signal="${1:?_fi_send_signal_at_phase: missing SIGNAL}"
  local phase_file="${2:?_fi_send_signal_at_phase: missing PHASE_FILE}"
  ...
  if [[ ! -f "$phase_file" ]]; then
    echo "ERROR: _fi_send_signal_at_phase: timed out waiting for phase marker: $phase_file" >&2
    return 1
  fi
  ...
  if [[ -z "$target_pid" || ! "$target_pid" =~ ^[0-9]+$ ]]; then
    echo "ERROR: _fi_send_signal_at_phase: invalid PID in phase marker: '$target_pid'" >&2
    return 1
  fi

  if ! kill -0 "$target_pid" 2>/dev/null; then
    echo "ERROR: _fi_send_signal_at_phase: process $target_pid is not running" >&2
    return 1
  fi

  if ! kill -"$signal" "$target_pid" 2>/dev/null; then
    echo "ERROR: _fi_send_signal_at_phase: failed to send $signal to PID $target_pid" >&2
    return 1
  fi
...
# ===========================================================================
# _fi_run_with_signal_at_phase FUNC [ARGS...] SIGNAL PHASE_NAME TIMEOUT_SECS
#
...
#   }
#   _fi_run_with_signal_at_phase my_func SIGTERM "phase-1" 5
#
...
# ===========================================================================
_fi_run_with_signal_at_phase() {
  local func="${1:?_fi_run_with_signal_at_phase: missing FUNC}"
  shift
  ...
  _fi_send_signal_at_phase "$signal" "$_FI_SIGNAL_PHASE_FILE" "$timeout_secs"
  local rc=$?
...
# ===========================================================================
# _fi_inject_rename_failure TARGET_DIR FILENAME
#
...
#
# The caller should invoke _fi_cleanup_rename_failure() to restore
# permissions after the test.
#
# Usage:
#   _fi_inject_rename_failure "$esp_dir/SteamOS/partsets"
#   # ... operations that attempt rename into this directory will fail ...
#   _fi_cleanup_rename_failure "$esp_dir/SteamOS/partsets"
#
# Arguments:
...
# ===========================================================================
_fi_inject_rename_failure() {
  local target_dir="${1:?_fi_inject_rename_failure: missing TARGET_DIR}"

  if [[ ! -d "$target_dir" ]]; then
    echo "ERROR: _fi_inject_rename_failure: directory does not exist: $target_dir" >&2
    return 1
  fi
  ...
  if ! chmod u-w "$target_dir"; then
    echo "ERROR: _fi_inject_rename_failure: chmod failed on $target_dir" >&2
    return 1
  fi

  # Register cleanup with test harness if available
  if declare -f test_harness_register_cleanup >/dev/null 2>&1; then
    test_harness_register_cleanup _fi_cleanup_rename_failure
  fi
...
# ===========================================================================
# _fi_cleanup_rename_failure [TARGET_DIR]
#
# Restore permissions after _fi_inject_rename_failure(). Idempotent.
#
# Arguments:
#   TARGET_DIR - Directory to restore (default: last injected directory)
# ===========================================================================
# shellcheck disable=SC2120  # $1 is optional
_fi_cleanup_rename_failure() {
...
# ===========================================================================
# _fi_inject_fsync_failure TARGET_FILE
#
...
#
# Usage:
#   _fi_inject_fsync_failure "$esp_dir/SteamOS/conf/B.conf"
#
# Arguments:
...
#   1 - Error injecting failure
# ===========================================================================
_fi_inject_fsync_failure() {
  local target_file="${1:?_fi_inject_fsync_failure: missing TARGET_FILE}"

  if [[ ! -f "$target_file" ]]; then
    echo "ERROR: _fi_inject_fsync_failure: file does not exist: $target_file" >&2
    return 1
  fi
  ...
  # Register cleanup
  if declare -f test_harness_register_cleanup >/dev/null 2>&1; then
    test_harness_register_cleanup _fi_cleanup_fsync_failure
  fi
...
# ===========================================================================
# _fi_cleanup_fsync_failure [TARGET_FILE]
#
...
#
# Usage:
#   _fi_inject_enospc "$esp_dir"
#   # ... operations that write will fail with ENOSPC ...
#   _fi_cleanup_enospc "$esp_dir"
#
# Arguments:
...
# ===========================================================================
_fi_inject_enospc() {
  local target_dir="${1:?_fi_inject_enospc: missing TARGET_DIR}"

  if [[ ! -d "$target_dir" ]]; then
    echo "ERROR: _fi_inject_enospc: directory does not exist: $target_dir" >&2
    return 1
  fi
  ...
  if [[ "$avail_kb" -le 1 ]]; then
    echo "WARNING: _fi_inject_enospc: filesystem already full or unavailable" >&2
    return 0
  fi
  ...
  if [[ "$fill_blocks" -gt 0 ]]; then
    if ! dd if=/dev/zero of="$_FI_ENOSPC_FILL_FILE" bs=1024 count="$fill_blocks" 2>/dev/null; then
      echo "WARNING: _fi_inject_enospc: dd fill may not have completed fully" >&2
    fi
  fi

  # Register cleanup
  if declare -f test_harness_register_cleanup >/dev/null 2>&1; then
    test_harness_register_cleanup _fi_cleanup_enospc
  fi
...
# ===========================================================================
# _fi_inject_transaction_marker EFI_DIR MARKER_NAME [SLOT]
#
...
#
# Usage:
#   _fi_inject_transaction_marker "$efi_dir" "grub-update" "B"
#
# Arguments:
...
# ===========================================================================
_fi_inject_transaction_marker() {
  local efi_dir="${1:?_fi_inject_transaction_marker: missing EFI_DIR}"
  local marker_name="${2:?_fi_inject_transaction_marker: missing MARKER_NAME}"
  local slot="${3:-B}"

  if [[ ! -d "$efi_dir" ]]; then
    echo "ERROR: _fi_inject_transaction_marker: EFI directory does not exist: $efi_dir" >&2
    return 1
  fi
...
MARKER_EOF
  ...
  if [[ ! -f "$marker_file" ]]; then
    echo "ERROR: _fi_inject_transaction_marker: failed to create marker file: $marker_file" >&2
    return 1
  fi

  # Register cleanup
  if declare -f test_harness_register_cleanup >/dev/null 2>&1; then
    test_harness_register_cleanup _fi_cleanup_transaction_markers
  fi
...
# ===========================================================================
# _fi_cleanup_transaction_markers [EFI_DIR]
#
...
# ===========================================================================
# shellcheck disable=SC2120  # $1 is optional
_fi_cleanup_transaction_markers() {
  local efi_dir="${1:-}"
...
# ===========================================================================
# _fi_verify_transaction_recovery EFI_DIR MARKER_NAME
#
...
#   _fi_verify_transaction_recovery "$efi_dir" "grub-update"
#
# Arguments:
...
# ===========================================================================
_fi_verify_transaction_recovery() {
  local efi_dir="${1:?_fi_verify_transaction_recovery: missing EFI_DIR}"
  local marker_name="${2:?_fi_verify_transaction_recovery: missing MARKER_NAME}"
  ...
  if [[ ! -f "$marker_file" ]]; then
    echo "ERROR: _fi_verify_transaction_recovery: marker not found: $marker_file" >&2
    return 1
  fi

  # 2. Validate JSON-like structure
  if ! grep -q '"transaction_id"' "$marker_file" 2>/dev/null; then
    echo "ERROR: _fi_verify_transaction_recovery: marker missing transaction_id field" >&2
    rc=1
  fi
  if ! grep -q '"operation"' "$marker_file" 2>/dev/null; then
    echo "ERROR: _fi_verify_transaction_recovery: marker missing operation field" >&2
    rc=1
  fi
  if ! grep -q '"status"' "$marker_file" 2>/dev/null; then
    echo "ERROR: _fi_verify_transaction_recovery: marker missing status field" >&2
    rc=1
  fi
  ...
  if [[ "$status" != "in-progress" ]]; then
    echo "ERROR: _fi_verify_transaction_recovery: expected status 'in-progress', got '$status'" >&2
    rc=1
  fi
  ...
  if [[ ${#stale_found[@]} -gt 0 ]]; then
    echo "ERROR: _fi_verify_transaction_recovery: stale artifacts remain alongside marker:" >&2
    printf '  %s\n' "${stale_found[@]}" >&2
    rc=1
  fi
...
# ===========================================================================
# _fi_inject_rollback_failure EFI_DIR SLOT
#
...
#
# Usage:
#   _fi_inject_rollback_failure "$efi_dir" "B"
#
# Arguments:
...
# ===========================================================================
_fi_inject_rollback_failure() {
  local efi_dir="${1:?_fi_inject_rollback_failure: missing EFI_DIR}"
  local slot="${2:?_fi_inject_rollback_failure: missing SLOT}"

  if [[ ! -d "$efi_dir" ]]; then
    echo "ERROR: _fi_inject_rollback_failure: EFI directory does not exist: $efi_dir" >&2
    return 1
  fi
  ...
  if [[ ${#protected_files[@]} -eq 0 ]]; then
    echo "WARNING: _fi_inject_rollback_failure: no backup files found to protect in $efi_dir" >&2
  fi

  # Register cleanup
  if declare -f test_harness_register_cleanup >/dev/null 2>&1; then
    test_harness_register_cleanup _fi_cleanup_rollback_failure
  fi
...
# ===========================================================================
# _fi_cleanup_rollback_failure [EFI_DIR]
#
...
# ===========================================================================
# _fi_inject_partial_activation EFI_DIR ESP_DIR SLOT PHASE
#
...
#
# Usage:
#   _fi_inject_partial_activation "$efi_dir" "$esp_dir" "B" "grub-only"
#
# Arguments:
...
# ===========================================================================
_fi_inject_partial_activation() {
  local efi_dir="${1:?_fi_inject_partial_activation: missing EFI_DIR}"
  local esp_dir="${2:-}"
  local slot="${3:?_fi_inject_partial_activation: missing SLOT}"
  local phase="${4:?_fi_inject_partial_activation: missing PHASE}"

  if [[ ! -d "$efi_dir" ]]; then
    echo "ERROR: _fi_inject_partial_activation: EFI directory does not exist: $efi_dir" >&2
    return 1
  fi
  ...
  case "$phase" in
    ...
    before-efi)
      ...
      ;;
    *)
      echo "ERROR: _fi_inject_partial_activation: unknown phase: $phase" >&2
      return 1
      ;;
  esac
  ...
  # Register cleanup
  if declare -f test_harness_register_cleanup >/dev/null 2>&1; then
    test_harness_register_cleanup _fi_cleanup_partial_activation
  fi
...
# ===========================================================================
# _fi_cleanup_partial_activation [EFI_DIR] [ESP_DIR]
#
...
# ===========================================================================
# shellcheck disable=SC2120  # $1 is optional
_fi_cleanup_partial_activation() {
  local efi_dir="${1:-}"
  local esp_dir="${2:-}"
...
# ===========================================================================
# _fi_verify_partial_activation_recovery EFI_DIR [ESP_DIR]
#
...
# ===========================================================================
_fi_verify_partial_activation_recovery() {
  local efi_dir="${1:?_fi_verify_partial_activation_recovery: missing EFI_DIR}"
  local esp_dir="${2:-}"
  ...
  if [[ -f "$grub_cfg" ]]; then
    ...
    if [[ "$partial_markers" -gt 0 ]]; then
      echo "ERROR: _fi_verify_partial_activation_recovery: grub.cfg still has partial-activation markers" >&2
      rc=1
    fi
  fi
  ...
  if [[ -d "$partsets_dir" ]]; then
    while IFS= read -r ps_file; do
      ...
      if [[ "$partial_count" -gt 0 ]]; then
        echo "ERROR: _fi_verify_partial_activation_recovery: partset $(basename "$ps_file") still has partial markers" >&2
        rc=1
      fi
  ...
  if [[ -d "$partsets_dir" ]]; then
    while IFS= read -r ps_file; do
      [[ -z "$ps_file" ]] && continue
      if [[ ! -s "$ps_file" ]]; then
        echo "ERROR: _fi_verify_partial_activation_recovery: partset $(basename "$ps_file") is empty" >&2
        rc=1
      fi
    ...
    if [[ -d "$conf_dir" ]]; then
      for conf_file in "$conf_dir"/*.conf; do
        ...
        if [[ -n "$invalid_val" && "$invalid_val" != "0" && "$invalid_val" != "1" ]]; then
          echo "ERROR: _fi_verify_partial_activation_recovery: $(basename "$conf_file") has invalid image-invalid value: '$invalid_val'" >&2
          rc=1
        fi
      done
    fi
...
# ===========================================================================
# _fi_snapshot_resources BASE_DIR FILE_PATHS...
#
...
#
# Usage:
#   _fi_snapshot_resources "$efi_dir" \
...
# ===========================================================================
_fi_snapshot_resources() {
  local base_dir="${1:?_fi_snapshot_resources: missing BASE_DIR}"
  shift
  local -a file_paths=("$@")

  if [[ ${#file_paths[@]} -eq 0 ]]; then
    echo "ERROR: _fi_snapshot_resources: no file paths provided" >&2
    return 1
  fi
  ...
  if [[ ! -d "$_FI_SNAPSHOT_DIR" ]]; then
    echo "ERROR: _fi_snapshot_resources: failed to create snapshot directory" >&2
    return 1
  fi
  ...
  for file_path in "${file_paths[@]}"; do
    if [[ ! -f "$file_path" ]]; then
      echo "WARNING: _fi_snapshot_resources: file does not exist: $file_path" >&2
      continue
    fi
  ...
  # Register cleanup
  if declare -f test_harness_register_cleanup >/dev/null 2>&1; then
    test_harness_register_cleanup _fi_cleanup_snapshot
  fi
...
# ===========================================================================
# _fi_verify_cleanup_ownership BASE_DIR SNAPSHOT_DIR FILE_PATHS...
#
# Verify that file ownership and permissions match the snapshot taken
# by _fi_snapshot_resources(). This is used to verify that cleanup
# operations restore files to their original ownership state.
#
# Usage:
#   _fi_verify_cleanup_ownership "$efi_dir" "$snapshot_dir" \
#     "$efi_dir/EFI/steamos/grub.cfg"
#
# Arguments:
#   BASE_DIR     - Base directory for relative path display
#   SNAPSHOT_DIR - Directory containing the snapshot from _fi_snapshot_resources
...
# ===========================================================================
_fi_verify_cleanup_ownership() {
  local base_dir="${1:?_fi_verify_cleanup_ownership: missing BASE_DIR}"
  local snapshot_dir="${2:?_fi_verify_cleanup_ownership: missing SNAPSHOT_DIR}"
  shift 2
  local -a file_paths=("$@")
  ...
  if [[ ! -f "$snapshot_file" ]]; then
    echo "ERROR: _fi_verify_cleanup_ownership: snapshot file not found: $snapshot_file" >&2
    return 1
  fi
  ...
  for file_path in "${file_paths[@]}"; do
    ...
    if [[ -z "$snapshot_entry" ]]; then
      echo "WARNING: _fi_verify_cleanup_ownership: no snapshot entry for: $rel_path" >&2
      continue
    fi
    ...
    # Current state
    if [[ ! -f "$file_path" ]]; then
      echo "ERROR: _fi_verify_cleanup_ownership: file missing after cleanup: $file_path" >&2
      rc=1
      continue
    fi
    ...
    if [[ "$cur_owner" != "$snap_owner" ]]; then
      echo "ERROR: _fi_verify_cleanup_ownership: owner mismatch for $rel_path (expected=$snap_owner, actual=$cur_owner)" >&2
      rc=1
    fi
    if [[ "$cur_group" != "$snap_group" ]]; then
      echo "ERROR: _fi_verify_cleanup_ownership: group mismatch for $rel_path (expected=$snap_group, actual=$cur_group)" >&2
      rc=1
    fi
    if [[ "$cur_perms" != "$snap_perms" ]]; then
      echo "ERROR: _fi_verify_cleanup_ownership: permissions mismatch for $rel_path (expected=$snap_perms, actual=$cur_perms)" >&2
      rc=1
    fi
    if [[ "$cur_checksum" != "$snap_checksum" ]]; then
      echo "ERROR: _fi_verify_cleanup_ownership: content checksum mismatch for $rel_path" >&2
      rc=1
    fi
  done
...
# ===========================================================================
# _fi_register_emergency_cleanup FUNC [ARGS...]
#
...
#
# Usage:
#   _fi_register_emergency_cleanup my_cleanup_func "arg1" "arg2"
#
# Arguments:
...
# ===========================================================================
_fi_register_emergency_cleanup() {
  local func="${1:?_fi_register_emergency_cleanup: missing FUNC}"
  shift
  local -a args=("$@")

  if ! declare -f "$func" >/dev/null 2>&1; then
    echo "ERROR: _fi_register_emergency_cleanup: '$func' is not a function" >&2
    return 1
  fi
  ...
  if declare -f test_harness_register_cleanup >/dev/null 2>&1; then
    test_harness_register_cleanup _fi_verify_emergency_cleanup
  fi
    ...
    if [[ "$args_str" == "$func_name" ]]; then
      ...
      if declare -f "$func_name" >/dev/null 2>&1; then
        "$func_name" 2>/dev/null || {
          echo "WARNING: _fi_verify_emergency_cleanup: handler '$func_name' failed" >&2
          rc=1
        }
      else
        echo "WARNING: _fi_verify_emergency_cleanup: handler function '$func_name' not found" >&2
        rc=1
      fi
    else
...
_fi_cleanup_all() {
  ...
  _fi_cleanup_rename_failure 2>/dev/null || true
  # shellcheck disable=SC2119  # args are optional
  _fi_cleanup_fsync_failure 2>/dev/null || true
  _fi_cleanup_enospc 2>/dev/null || true
  # shellcheck disable=SC2119  # args are optional
  _fi_cleanup_transaction_markers 2>/dev/null || true
  # shellcheck disable=SC2119  # args are optional
  _fi_cleanup_rollback_failure 2>/dev/null || true
  # shellcheck disable=SC2119  # args are optional
  _fi_cleanup_partial_activation 2>/dev/null || true
  _fi_cleanup_snapshot 2>/dev/null || true
  _fi_verify_emergency_cleanup 2>/dev/null || true
}
